...
background = pygame.transform.scale(pygame.image.load(os.path.join(asset_folder, 'black.png')),(SCREEN_WIDTH, SCREEN_HEIGHT))
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.radius = int((1.0/4.0)*numpy.sqrt(self.scale[0]**2+self.scale[1]**2))
    ...
    def move(self):
        ...
        if self.rect.left   < X_BUFFER:
            self.rect.left  = X_BUFFER
        if self.rect.right  > SCREEN_WIDTH - X_BUFFER:
            self.rect.right = SCREEN_WIDTH - X_BUFFER
        ...
        if self.rect.top < Y_BUFFER:
            self.rect.top = Y_BUFFER
        if self.rect.bottom > SCREEN_HEIGHT - Y_BUFFER:
            self.rect.bottom = SCREEN_HEIGHT - Y_BUFFER
...
class Mob(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.scale = (int(SCREEN_WIDTH*SCALE_WIDTH), int(SCREEN_HEIGHT*SCALE_LENGTH))
        self.speedy = random.randrange(int(SCREEN_HEIGHT*ENEMY_SPEED_SCALEY/2.0), int(SCREEN_HEIGHT*ENEMY_SPEED_SCALEY))
