...
class Mob(pygame.sprite.Sprite):
    def __init__(self, enemy_img, bullet_img, SCREEN_WIDTH, SCREEN_HEIGHT):
        pygame.sprite.Sprite.__init__(self)

        self.SPEED_SCALEX = 1.0/100.0
        self.SPEED_SCALEY = 1.0/60.0
        ...
        self.bulletspeedx = 0.5*self.speedx
        self.bulletspeedy = 0.5*self.speedy 
        self.last_shoot = pygame.time.get_ticks()
        self.now = pygame.time.get_ticks()

    def shoot(self):
        bullet = Bullet(
            self.bullet_img, int(self.rect.x+self.rect.w/2.0), 
            int(self.rect.top), self.bulletspeedx, self.bulletspeedy,
